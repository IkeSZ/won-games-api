...
const exception = (err) => {
  return { err, data: err.data && err.data.errors && err.data.errors };
};
...
const getGameInfo = async (slug) => {
  try {
    ...
    return {
      ...
      short_description: description.textContent.trim(),
    };
  } catch (err) {
...
const createManyToManyData = async (products) => {
  ...
  try {
    ...
    console.log('createManyToManyData', exception(err));
  }
};
...
const createGames = async (products) => {
  try {
    ...
    console.log('crateGames', exception(err));
  }
};
...
module.exports = {
  populate: async (params) => {
    try {
      ...
      console.log('populate', exception(err));
    }
  }
};
